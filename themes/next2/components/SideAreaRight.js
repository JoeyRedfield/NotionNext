...
const SideAreaRight = (props) => {
  ...
  return (<aside id='right' className={(JSON.parse(siteConfig('LAYOUT_SIDEBAR_REVERSE')) ? 'mr-4' : 'ml-4') + ' space-y-4 hidden xl:block flex-col w-64 relative z-10'}>
                ...
                <Card>
                    <div className='text-sm px-2 flex flex-nowrap justify-between font-light'>
                        ...
                        <SmartLink
                            href={'/category'}
                            passHref
                            className='next-link-accent text-gray-500 dark:text-gray-400 hover:underline cursor-pointer'>

                            {locale.COMMON.MORE} <i className='fas fa-angle-double-right' />

                        </SmartLink>
                    </div>
                    <CategoryGroup currentCategory={currentCategory} categories={categoryOptions} variant='default' />
                </Card>
            )}
